...
def main():
    ...
    for song_dir in os.listdir(base_dir):
        ...
        if not os.path.exists(vocals_file) or not os.path.exists(instrument_file):
            print(f"Skipping {song_dir}: Missing required files")
            continue
            
        print(f"\nProcessing song: {song_dir}")
        
        # Create output directory for this song
        output_dir = os.path.join("instrumental_breaks", song_dir)
        ...
        try:
            ...
            silent_regions = detect_vocal_breaks(
                vocals_file,
                instrument_file,
                min_silence_duration=7,     # 7 seconds minimum silence
                silence_threshold=0.05      # Adjust based on your needs
            )
            
            # Move or copy the visualization to the song's output directory
            if os.path.exists("breaks_detection_visualization.png"):
                os.rename(
                    "breaks_detection_visualization.png",
                    os.path.join(output_dir, "breaks_detection_visualization.png")
                )
            
            # Move the generated break files to the song's directory
            for i in range(len(silent_regions)):
                break_file = f"instrument_break_{i+1}.wav"
                if os.path.exists(break_file):
                    os.rename(
                        break_file,
                        os.path.join(output_dir, break_file)
                    )
            
            print(f"Successfully processed {song_dir}: Found {len(silent_regions)} breaks")
